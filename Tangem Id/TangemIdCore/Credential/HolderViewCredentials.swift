//
//  HolderViewCredentials.swift
//  Tangem Id
//
//  Created by Andrew Son on 10/22/20.
//  Copyright © 2020 Tangem AG. All rights reserved.
//

import Foundation
import TangemSdk

class HolderCredential<T: DemoCredential> {
	let credentials: T
	let file: File
	let json: String
	
	internal init(credentials: T, file: File, json: String) {
		self.credentials = credentials
		self.file = file
		self.json = json
	}
}

extension FileSettings {
	mutating func toggleVisibility() {
		self = self == .public ? .private : .public
	}
}

class HolderViewCredentials: NSCopying {
	
	let cardId: String
	var photo: HolderCredential<PhotoCredential>?
	var personalInfo: HolderCredential<PersonalInfoCredential>?
	var ssn: HolderCredential<SsnCredential>?
	var ageOver21: HolderCredential<AgeOver21Credential>?
	var covid: HolderCredential<CovidCredential>?
	
	internal init(cardId: String, photo: HolderCredential<PhotoCredential>? = nil, personalInfo: HolderCredential<PersonalInfoCredential>? = nil, ssn: HolderCredential<SsnCredential>? = nil, ageOver21: HolderCredential<AgeOver21Credential>? = nil, covid: HolderCredential<CovidCredential>? = nil) {
		self.cardId = cardId
		self.photo = photo
		self.personalInfo = personalInfo
		self.ssn = ssn
		self.ageOver21 = ageOver21
		self.covid = covid
	}
	
	func copy(with zone: NSZone? = nil) -> Any {
		return HolderViewCredentials(cardId: cardId, photo: photo, personalInfo: personalInfo, ssn: ssn, ageOver21: ageOver21, covid: covid)
	}
	
	static var demo = HolderViewCredentials(
		cardId: "BB03 0000 0000 0004",
		photo: HolderCredential<PhotoCredential>(credentials: PhotoCredential(photo: Data(base64Encoded: "") ?? Data()),
												 file: File(fileIndex: 0, fileSettings: .public, fileData: Data()),
												 json: "{\n}"),
		personalInfo: HolderCredential<PersonalInfoCredential>(credentials: PersonalInfoCredential(name: "Tangem",
																								   surname: "Holder",
																								   gender: "Other",
																								   dateOfBirth: "10/04/2020"),
															   file: File(fileIndex: 1, fileSettings: .private, fileData: Data()),
															   json: "{\n    \"name\": \"Tangem\"\n}"),
		ssn: HolderCredential<SsnCredential>(credentials: SsnCredential(ssn: "000-56-2456"),
											 file: File(fileIndex: 2, fileSettings: .public, fileData: Data()),
											 json: ""),
		ageOver21: HolderCredential<AgeOver21Credential>(credentials: AgeOver21Credential(isOver21: false),
														 file: File(fileIndex: 3, fileSettings: .public, fileData: Data()),
														 json: ""),
		covid: HolderCredential<CovidCredential>(credentials: CovidCredential(isCovidPositive: false),
												 file: File(fileIndex: 4, fileSettings: .private, fileData: Data()),
												 json: "{\n\n\n}")
		)
}
